...
class XTSConnect(XTSCommon):
    ...
    _routes = {
        ...
        "intraday.profitandloss": "interactive/portfolio/positions?dayOrNet=DayWise",
        ...
        "portfolio.positions.profitandLoss": "/interactive/portfolio/positions?dayOrNet = DayWise",
    ...
    def get_profitandloss(self, clientID=None):
        """Get Balance API call grouped under this category information related to limits on equities, derivative,
        upfront margin, available exposure and other RMS related balances available to the user."""
        response = None
        if True:
            try:
                ...
                if not self.isInvestorClient:
                    params['clientID'] = clientID
                response = self._get('intraday.profitandloss', params)
                return response
            except Exception as e:
                if response is not None:
                    return response['description']
